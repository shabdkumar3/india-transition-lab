...
export default function DemandPage() {
  ...
  const [openDeriv, setOpenDeriv] = useState<string | null>(null);
          ...
          {s.demandTrajectories.map((t, idx) => {
            ...
            return (
              <div key={t.key} style={{ ...CARD, borderRadius:12, overflow:"hidden" }}>
                ...
                <button
                  onClick={() => setOpenDeriv(prev => prev === t.key ? null : t.key)}
                  style={{
                ...
                >
                  <span style={{ display:"flex", alignItems:"center", gap:8 }}>
                    ...
                    <span style={{ fontSize:11, fontWeight:700, color:T.sub }}>How this number was derived</span>
                    <span style={{ fontSize:10, color:T.dim }}>· calculation steps, sources, assumptions</span>
                  </span>
                  <span style={{
                    fontSize:13, color:T.dim,
                    transform: openDeriv === t.key ? "rotate(180deg)" : "rotate(0deg)",
                    transition:"transform 200ms",
                  }}>▾</span>
                </button>

                {openDeriv === t.key && t.derivation && (
                  <div style={{ padding:"20px 24px", borderTop:`1px solid ${T.border}`, background:T.bg }}>
                    <DerivationBody text={t.derivation} trajColor={t.color} />
                  </div>
                )}
              </div>
            );
